...
export default function Minting() {
	...
	async function claimNFTs() {
		if (active && account) {
			...
			try {
				...
				setIsMinting(false)
				setMessage(
					`Yay! ${mintAmount} ${process.env.NEXT_PUBLIC_NFT_SYMBOL} successfully sent to ${account.substring(0, 6)}...${account.substring(
						account.length - 4
					)}`
				)
			} catch (error) {
	...
	return (
		<>
			<h2 className="mb-4 text-4xl">MINTING</h2>

			{active && account ? (
				<div className="mb-4 space-y-4">
					<div className="p-8 space-y-4 bg-black rounded border-t-red-600 border-r-red-600 border-b-red-600 border-l-red-700">
						...
						<div className="flex items-center justify-center space-x-4">
							<IconContext.Provider value={{ size: '1.5em' }}>
								...
								<button
									type="button"
									className={mintAmount === 30 ? 'text-red-600 cursor-default' : ''}
						...
						<div className="flex justify-center">
							{!active || errMsg ? (
								<button
									type="button"
									className={`rounded px-4 py-2 bg-black font-bold w-40 cursor-not-allowed`}
									disabled={true}
									onClick={claimNFTs}
								>
									Buy
								...
								<>
									{isPending || isMinting ? (
										...
										<button type="button" className={`rounded px-4 py-2 bg-red-600 hover:bg-red-800 font-bold w-40`} onClick={claimNFTs}>
											Buy
										</button>
									)}
								</>
					...
					<div className="space-y-4">
						<p>
							Please make sure you are connected to the correct address and the correct network (Polygon Mainnet) before purchasing. The
							operation cannot be undone after purchase.
						</p>
						<p>We have set the gas limit to 285000 to successfully mint your NFT. We recommend that you do not lower the gas limit.</p>
					</div>
				</div>
			) : (
				<div className="p-8 border-4 border-dashed rounded border-black-200">
					<ConnectButton />
				</div>
			)}
		</>
